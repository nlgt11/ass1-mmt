...
class Client:
    ...
    def createWidgets(self):
        ...
        self.label_data = Label(self.master, height=1,
                                textvariable=self.rcvStr)
        self.label_data.grid(row=2, column=0, columnspan=1)

        self.pckLost = 0
        self.pckLostStr = StringVar()
        self.label_pckLost = Label(
            self.master, height=1, textvariable=self.pckLostStr)
        self.label_pckLost.grid(row=2, column=1, columnspan=1)

        self.lossRate = 0
        self.lossRateStr = StringVar()
        self.label_loss = Label(self.master, height=1,
                                textvariable=self.lossRateStr)
        self.label_loss.grid(row=2, column=2, columnspan=1)

        self.totalRcvBytes = 0
        self.totalRcvBytesStr = StringVar()
        self.label_totalRcvBytes = Label(
            self.master, height=1, textvariable=self.totalRcvBytesStr)
    ...
    def updateText(self):
        ...
        self.lossRateStr.set(
            "Loss rate: {:0.2f} %".format(self.lossRate * 100))
        self.totalRcvBytesStr.set(
            "Totol recieved data: " + str(self.totalRcvBytes) + " bytes")
        self.dataRateStr.set(
            "Data rate: {:0.4f} bytes/s".format(self.dataRate))
            ...
            try:
                ...
                if data:
                    ...
                    if currFrameNbr > self.frameNbr:  # Discard the late packet
                        ...
                        self.rcv += 1
                        self.lossRate = self.pckLost / \
                            (self.pckLost + self.rcv)
                        ...
                        self.totalTime = int(time()) - \
                            self.startTime + self.accumTime
                        print("Time {}".format(self.totalTime))
                        self.dataRate = (
                            self.totalRcvBytes / self.totalTime) if self.totalTime != 0 else 0
    ...
    def sendRtspRequest(self, requestCode):
        ...
        if requestCode == self.SETUP and self.state == self.INIT:  # Case Set up
            ...
            request = self.requestTemp.format(
                requestCode, self.fileName, self.rtspSeq, self.sessionID)
